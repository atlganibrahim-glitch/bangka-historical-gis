...
Scripts are in [`v3/`](v3/). The pipeline can produce two output sets:

| set | rasters | metadata | what it is |
|---|---|---|---|
| **v3.1** (published, recommended) | `GEOREF_V3_1/` | `bangka_dataset_v3_1.csv` | v3.0 plus the inland road-network offset (§5) |
| v3.0 | `GEOREF_V3/` | `v3/bangka_dataset_v3.csv` | graticule-exact, no empirical shift |

Only **v3.1** is published on Hugging Face. v3.0 differs from it by a pure
translation of +200.4 m E / −66.8 m N, identical on all 176 sheets, so it can
be derived from the published set by shifting it — or regenerated by setting
the correction to zero in `v3/grid.py` and re-running `v3/georeference.py`.
...
`GEOREF_V3_1/` + `bangka_dataset_v3_1.csv` is the published set; the
uncorrected `GEOREF_V3/` + `v3/bangka_dataset_v3.csv` is generated locally by
the same pipeline but not uploaded. Every v3.1 raster is tagged
`VERSION=v3.1`, and all 176 carry exactly the same shift (σ = 0.00 m), so the
two sets differ by a pure translation and nothing else.
